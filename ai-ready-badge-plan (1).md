...
- **Caveat line, always shown (pass or fail), placed right below the crawler table, before the embed code section:** *"This confirms whether AI crawlers can access your site — it doesn't guarantee your content is cited in AI-generated answers, which also depends on content quality, competing sources, and each engine's own ranking."* This exists specifically to stop a "pass" badge from being over-read as "I now show up in ChatGPT/Perplexity answers" — the badge only ever measures crawler permission, not citation outcome. Community feedback (via a Build in Public thread) flagged this exact gap, which is why it's locked in as always-on rather than optional copy.
...
- **Badge generation:** a Route Handler at `app/badge/[domain]/route.ts` with this exact logic:
  ...
  - **Badge text includes the last-checked date**, e.g. "✅ AI-Search Ready · checked 2h ago", rendered from the cached row's `checked_at` timestamp. Trivial addition since `checked_at` is already stored — this was also community-suggested feedback, since a badge with no visible freshness signal can go stale-looking without the viewer knowing.
...
- ✅ Badge displays last-scan date (community feedback, "AEO Agency" via Build in Public thread)
- ✅ Report page always shows a caveat distinguishing "crawlable" from "cited in AI answers" (community feedback, same thread)
